...
# --- Loss ---
def u_target(x, k, lamb, b=1.0):
    #u_tilde = jnp.exp(-k * lamb * 0.0) * jnp.cos(lamb * 0.0)
    #const = b - u_tilde
    return jnp.exp(-k * lamb * x) * jnp.cos(lamb * x) #+ const
...
def train():
    ...
    output_dir = f"plotsA_bond_{bond_dimension}/index_{init_index}"
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0:
            #f = func_and_deriv(x_train, num_qubits, d, theta, bond_dimension)
            f0 = f_x(0.0, theta, num_qubits, d, bond_dimension)
    ...
    #pred = func_and_deriv(x_test, num_qubits, d, theta, bond_dimension)
    f0 = f_x(0.0, theta, num_qubits, d, bond_dimension)
